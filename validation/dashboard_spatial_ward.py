...
# Load labels
iteration = 0
labels = pd.read_csv(f"{config.output_dir_clustering}/labels_spatial_ward.csv")
labels = prepare_labels_df(labels, iteration=iteration)
hyp_columns = ["n_clusters", "linkage", "connectivity"]

# Score data
data = pd.read_csv(f"{config.output_dir_clustering}internal_validation_spatial_ward.csv")
data = data.drop(["clustering"], axis=1)
groupby_cols = ["preprocessing"] + hyp_columns
data = data.groupby(groupby_cols).mean().drop("iteration", axis=1).reset_index()  # Average over iterations
data = data.sort_values(hyp_columns)
